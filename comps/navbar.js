        ...
        justifyContent={!username?'flex-end':'space-around'}>
          ...
          {username && (
            <>
              ...
              <Link href={`/${username}`}><Image cursor={'pointer'} borderRadius={'50%'} w={10} src={user?.photoURL}/></Link>
              <Link href={'/'}>
                <Button onClick={()=>signOut(auth)} colorScheme={"purple"} type="ghost">Logout</Button>
              </Link>
            </>
          )}
        </Flex>
